...
public interface IPodService : IService
{
    ...
    Task<PodEntity> AddPodEntityAsync(string selector, IEnumerable<PodMetrics>? metrics, CancellationToken ct = default);
    ...
    Task<PodMetrics> AddOrUpdatePodMetricsAsync(Guid entityId, string label, string podSelector, double cpuUsage, double memoryUsage, CancellationToken ct = default);
    Task<bool> DeletePodMetricsAsync(Guid entityId, string podSelector, string? label, CancellationToken ct = default);
}